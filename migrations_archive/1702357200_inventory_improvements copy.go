...
func init() {
	m.Register(func(db dbx.Builder) error {
			...
			questions.Schema = schema.NewSchema(
				...
				&schema.SchemaField{
					...
					Options: &schema.SelectOptions{
						Values: []string{"text", "number", "select", "multiselect", "date", "boolean"},
					},
				},
				&schema.SchemaField{
					Name:     "options",
		...
		if err != nil {
			...
			queue.ListRule = &queueRule
			queue.ViewRule = &queueRule
			queue.CreateRule = &queueRule
			queue.UpdateRule = &queueRule
			queue.DeleteRule = &queueRule
		...
		queue.DeleteRule = &defaultRule
		queue.CreateRule = &defaultRule
		queue.UpdateRule = &defaultRule
		queue.ListRule = &defaultRule
		queue.ViewRule = &defaultRule
